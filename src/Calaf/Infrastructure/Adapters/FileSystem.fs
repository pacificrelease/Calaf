...
module private Directory =        
    ...
    let workspace
        ...
        result {
            ...
            return Gateway.toWorkspaceDirectoryInfo dirInfo changelog projects
        }
            
type FileSystem() =
    interface IFileSystem with
        member _.tryReadDirectory directory searchPatterns changelogFilename =
            Directory.workspace directory searchPatterns changelogFilename
        ...
        member _.tryReadMarkdown absolutePath=
            Markdown.load absolutePath
            |> Result.mapError CalafError.Infrastructure
            
        member _.tryWriteXml (absolutePath, content) =
            Xml.save absolutePath content
            |> Result.mapError CalafError.Infrastructure
            |> Result.map ignore
            
        member _.tryWriteMarkdown (absolutePath, content) =
            Markdown.write absolutePath content
            |> Result.mapError CalafError.Infrastructure
            |> Result.map ignore